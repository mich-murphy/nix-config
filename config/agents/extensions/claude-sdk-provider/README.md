...
- An invalid `pi_call` request — an inner `name` the current turn's tool catalog doesn't recognize, the self-referential `name: "pi_call"`, or missing/malformed `arguments` — is also just denied, with a `permissionDecisionReason` that tells the model what went wrong and how to correct it. A deny does not end `query()`, so the model can retry within the same turn instead of losing it outright. The runner caps this at 3 invalid attempts per turn; past the cap, the error surfaces only if the turn ends with zero captured valid calls, and a genuine disagreeing SDK result always wins over a captured call regardless — see below.
