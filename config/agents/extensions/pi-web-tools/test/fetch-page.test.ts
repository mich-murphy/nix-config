...
test("FetchPage requests non-2xx bodies only for raw HTML mode", async () => {
	const publicWeb = new FakePublicWebClient(ok(response("application/json", '{"error":"invalid"}', 422)));
	const service = new FetchPage({ publicWeb, settings: testFetchSettings });

	const result = await service.fetch({ url: requestUrl, format: "html" });

	assert.equal(result._tag, "ok");
	assert.equal(result.value._tag === "Text" ? result.value.text : "", '{"error":"invalid"}');
	assert.equal(publicWeb.requests[0]?.rejectHttpErrors, false);
});
...
test("FetchPage rejects PDFs as unsupported binary content", async () => {
	const publicWeb = new FakePublicWebClient(ok(response("application/pdf", Buffer.from("%PDF-1.7"))));
	const service = new FetchPage({ publicWeb, settings: testFetchSettings });
	...
	assert.deepEqual(result, {
		_tag: "err",
		error: { _tag: "UnsupportedBinaryContent", mime: "application/pdf" },
	});
});

function response(contentType: string, body: string | Buffer, status = 200): PublicWebResponse {
