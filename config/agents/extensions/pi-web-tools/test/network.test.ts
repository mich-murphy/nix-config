...
import { FetchPublicWebClient, classifyMimeType, createPinnedLookup, isPrivateOrLocalIp, parseContentType } from "../network.ts";
...
test("createPinnedLookup retains the validated DNS snapshot when later DNS state changes", async () => {
	const validatedAddresses = ["93.184.216.34"];
	const pinnedLookup = createPinnedLookup(validatedAddresses);
	validatedAddresses[0] = "127.0.0.1";

	const result = await new Promise<{ address: string; family: number }>((resolve, reject) => {
		pinnedLookup("example.com", { family: 4 }, (error, address, family) => {
			if (error) { reject(error); return; }
			if (typeof address !== "string") { reject(new Error("Expected one pinned address")); return; }
...
test("FetchPublicWebClient returns bounded non-2xx bodies only when requested", async () => {
	const server = await startServer((_request, response) => {
	...
	try {
		const client = new FetchPublicWebClient();
		const raw = await client.get(makeRequest(server.origin, { blockPrivateHosts: false, rejectHttpErrors: false }));
		const readable = await client.get(makeRequest(server.origin, { blockPrivateHosts: false, rejectHttpErrors: true }));

		assert.equal(raw._tag, "ok");
		assert.equal(raw.value.status, 422);
		assert.equal(raw.value.body.toString("utf8"), '{"error":"invalid input"}');
		assert.equal(readable._tag, "err");
		assert.equal(readable.error._tag, "HttpStatusRejected");
	} finally {
		await server.close();
	}
});
...
function makeRequest(
	url: string,
	overrides: { readonly blockPrivateHosts?: boolean; readonly maxResponseBytes?: number; readonly fallbackUserAgent?: string; readonly rejectHttpErrors?: boolean } = {},
): PublicWebRequest {
	...
	return {
		...
		rejectHttpErrors: overrides.rejectHttpErrors,
	};
}
