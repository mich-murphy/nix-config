...
The local daemon uses localhost, which an agent sandbox may block. If the first
`hunk session list --json` returns an empty session list or reports no active
sessions, retry that command once with network/sandbox escalation when the
environment supports it. Prefer a narrowly scoped reusable approval for
`hunk session` commands. Ask the user to launch Hunk only when the escalated
retry also finds no session, or when escalation is unavailable.

## Workflow

```text
1. hunk session list --json                             # find live sessions; escalate once if empty
...
- **Empty `sessions` or "No active Hunk sessions"** -- do not immediately conclude that Hunk is closed. Retry `hunk session list --json` once with network/sandbox escalation because the daemon uses localhost. Ask the user to open Hunk only if that retry is also empty or escalation is unavailable.
