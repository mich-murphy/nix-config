...
pub trait Process {
    ...
    /// Starts `request` detached, in its own process group with its output
    /// appended to `log`, and returns its pid without waiting. `stdin` and
    /// `timeout_seconds` are ignored: nothing waits on a detached process.
    fn spawn(&self, _request: &ProcessRequest, _log: &Path) -> Result<u32, PortError> {
        Err(PortError("detached spawn is unsupported".into()))
    }
}
...
impl Process for SystemProcess {
    ...
    fn spawn(&self, request: &ProcessRequest, log: &Path) -> Result<u32, PortError> {
        let open = |path: &Path| {
        ...
        let (stdout, stderr) = (open(log)?, open(log)?);
