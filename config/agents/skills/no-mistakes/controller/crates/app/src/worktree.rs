...
use domain::{
    authority::{self, AuthorityError, Grant},
    delivery::Delivery,
...
pub(super) fn validate_slot(
    ...
    {
        return Err(AuthorityError::SlotUnfinishedOwner);
    }
    let historical = !owners.is_empty();
    match (historical, authority) {
        (false, None) => Ok(false),
        (true, Some(id)) => validate_reuse(state, task, slot, id).map(|()| true),
        (true, None) => Err(AuthorityError::SlotReuseRequiresGrant),
        (false, Some(_)) => Err(AuthorityError::SlotReuseRequiresHistorical),
    }
}
...
fn validate_reuse(
    ...
    let value = state
        .tasks
        .get(task)
        .ok_or(AuthorityError::SlotOwnerUnknown)?;
    let grant = value
        .authorities
        .iter()
        .find(|entry| entry.id == id)
        .ok_or(AuthorityError::UnknownAuthorityId)?;
    ...
    if unstarted {
        ...
        Err(AuthorityError::SlotReuseRequiresUnstartedDelivery)
    }
}
...
pub(super) fn bind_worktree(
    ...
    let ctx = app.ctx("bind-slot", Some(task));
    let state = app
        .services
        .vcs
        .inspect_slot(slot)
    ...
    if !safe {
        return Err(ctx.reject(ConflictReason::SlotUnsafe(format!("{state:?}"))));
    }
    if check {
        return Ok(());
    }
    ...
    result.map_err(|error| ctx.reject(Rejection::External(error.0)))
}
...
pub(super) fn validate_paths(
    ...
    let ctx = app.ctx("snapshot", Some(task));
    let commits = app
        .services
        .vcs
        .commit_paths(base, head)
        .map_err(|error| ctx.reject(Rejection::External(error.0)))?;
    domain::delivery::paths_allow(delivery, &commits)
        .map_err(|_error| ctx.reject(AuthorityError::PathScopeRejected))
}
