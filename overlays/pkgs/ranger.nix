...
python3Packages.buildPythonApplication rec {
  ...
  '' + lib.optionalString imagePreviewSupport ''
    ...
    # configure image previews for kitty and change default config to preferred options
    substituteInPlace ranger/config/rc.conf \
      --replace "set preview_images false" "set preview_images true"
      --replace "set preview_images_method w3m" "set preview_images_method kitty"
      --replace "set confirm_on_delete multiple" "set confirm_on_delete never"
      --replace "set line_numbers false" "set line_numbers relative"
  '';
